...
seo:
  title: "Binance Design System for React — Yellow #FCD535, BinanceNova, 36 components"
  metaDescription: "Binance's design system as a DESIGN.md file. Yellow #FCD535, BinanceNova + BinancePlex, 22 colors, 36 components. For React, Next.js, and AI tools."
  tags:
    - "Fintech & Crypto"
    - "Banking & Payments"
  highlights:
    - "Single yellow voltage — #FCD535 with #181a20 black text is the only primary CTA combination, never inverted"
    - "Two-typeface split — BinanceNova for editorial, BinancePlex for every price, volume, and stat counter"
    - "Dual-theme surfaces — dark #0b0e11 for marketing, white #ffffff for buy/deposit transactional flows"
  ...
  questions:
    - id: "primary-color"
      title: "What is Binance's primary brand color?"
      answer: "Binance's brand color is Binance Yellow — #FCD535, a saturated mid-yellow that carries every primary CTA, the wordmark, brand-claim headlines like 'FUNDS ARE SAFU', and the largest stat numbers on the homepage. It pairs with #181a20 black text on the button surface (never white on yellow — that combination is a system violation). The press state darkens to #f0b90b, and disabled flattens to a desaturated #3a3a1f tint on dark canvas. There is no secondary brand accent; a single turquoise (#2dbdb6) appears on the Smart Money product but is treated as a one-off experiment rather than a system token."
    - id: "typography"
      title: "What typography does Binance use, and what should I use if BinanceNova isn't available?"
      answer: "Binance runs two custom typefaces: BinanceNova for editorial (headlines, body, buttons, nav) and BinancePlex for numerical data (prices, volumes, percentages, stat counters). The split is functional, not decorative — putting BinancePlex on a paragraph reads monospace-cold, putting BinanceNova on a price ticker loses the trading-platform character. Display weights sit at 600–700, body at 400. If BinanceNova is unavailable, Inter is the closest open-source substitute; for BinancePlex use JetBrains Mono or IBM Plex Sans depending on whether tabular fidelity matters more than humanist proportions. Adjust display line-heights down by ~3% to match BinanceNova's tighter cap height."
    - id: "dual-theme"
      title: "Why does Binance use two canvas modes?"
      answer: "Marketing surfaces (homepage, Smart Money, Futures Arena) default to dark — canvas #0b0e11, surface cards #1e2329 — because dark feels authoritative for a trading platform. Transactional surfaces (buy crypto, deposit, withdraw) flip to light — canvas #ffffff with #f5f5f5 form fills — because forms read more cleanly on white and reduce cognitive load during money movement. The yellow CTAs and trading green (#0ecb81) / red (#f6465d) tokens carry through both modes unchanged, which is what makes the brand recognisable across the flip. A distinctive quirk: even dark-canvas pages close with a #fafafa light footer, deliberately inverting the surface to act as a marketing reset."
    - id: "use-in-project"
      title: "Can I use this DESIGN.md to build my own React trading dashboard?"
      answer: "Yes — the file is designed to be fed into Claude, Cursor, or any AI tool that reads structured design tokens. The agent will reproduce Binance's exact rules: yellow scarcity, BinancePlex for numbers, trading green and red as text-only color, flat surfaces with color-block separation. You can also reference the tokens directly: every color, type style, radius, and spacing value is a quoted hex or unit you can paste into Tailwind config, CSS variables, or your own component library. The 36 components cover top nav, markets tables, trust badges, FAQ rows, buy-crypto cards, and the price-chart card needed to build out a working exchange surface."
    - id: "trading-semantics"
      title: "How does Binance handle price-direction colors?"
      answer: "Trading green (#0ecb81) and red (#f6465d) are reserved semantic tokens — they signal price direction in tables, charts, and inline tickers and are applied as text color rather than card-surface fill. The one exception is the Buy / Long and Sell / Short action buttons, which use the green and red as backgrounds at small size (4px radius, 8×20px padding) inside dense trading interfaces. Never repurpose them for generic 'success' or 'error' states — that conflates UI feedback with price meaning and breaks the trading-platform contract."
    - id: "known-gaps"
      title: "What's missing from this DESIGN.md spec?"
      answer: "A handful of items documented in the Known Gaps section: the BinanceNova / BinancePlex variable-font weight axis is not formalized — only the static weights observed in screenshots are documented. Animation timings (chart redraws, price-change flashes) are out of scope. Form validation error and success states beyond the default text-input-on-light were not extracted because they require a sign-up or order flow to surface. The full Spot / Futures / Margin trading dashboards (order book layout, candlestick chart configuration, position-management cards) were not in the analyzed URL set, so they aren't documented here. The light/dark theme toggle behavior is product behavior, not extracted from marketing surfaces."
