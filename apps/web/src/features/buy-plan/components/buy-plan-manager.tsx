...
import { BuyPlanInsights } from '@/features/buy-plan/components/buy-plan-insights';
import { BuyPlanSuggestion } from '@/features/buy-plan/components/buy-plan-suggestion';
...
export function BuyPlanManager({ plans, currentPrices, recommendation, analysis }: Props) {
  ...
  function handleApplySuggestions(
    suggestedLevels: readonly { targetPrice: number; investmentAmount: number }[],
  ): void {
    setLevels(
      suggestedLevels.map((lvl) => ({
        targetPrice: lvl.targetPrice.toString(),
        investmentAmount: lvl.investmentAmount.toString(),
      })),
    );
    setMessage('นำราคาซื้อแนะนำ 3 ไม้ใส่ในฟอร์มแล้ว');
  }
  ...
  return (
    <div className="buy-plan-container">
      <BuyPlanInsights analysis={analysis} recommendation={recommendation} />
      ...
      <div className="buy-plan-workspace">
        <section aria-labelledby="buy-plan-form-title" className="transaction-panel">
